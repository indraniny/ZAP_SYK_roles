...
# =============================================================================
    ...
    K2=0
    K3=10**(2.813409983) #ZAP concentration
   
    #K3=10**(2.918527845) #ZAP concentration
    ...
    n=3#average of howmany bionetgen run
...
#0. Bionetgen parameter set 
    ...
    #model = bionetgen.bngmodel("BioModel_ZAP70_SYK_v8_test1i.bngl") #homodimer-gamma code
...
#2.Interpolate pSYKP  from 600 points to 2000 points   
    ...
    #plt.plot(T,avg_SYK,'b*',tnew,PSYK,'r-') 
...
#2.Interpolate PZAP  from 600 points to 2000 points   
   ...
   # plt.plot(T,avg_ZAP,'m*',tnew,PZAP,'g-') 
    ...
    file_path = os.path.join(folder_name, "ZAP_only.txt")
...
# # #5. Ca Signal from the ODE model uisng the PZAP signal   ##########PZAP  
  ...
  #7. Finding objective function only at t>60 sec
   ...
   # plt.plot(TT,EXPT,color=cmap(0.8),linewidth=3.0)
    ...
    # Write the data to a file in the folder
    file_path = os.path.join(folder_name, "ZAP_Ca.txt")
    with open(file_path, 'w') as file:
        for i in range(len(time)):
            file.write(f"{time[i]} {exp_data[i]}\n")
            
     # Write the data to a file in the folder
    file_path = os.path.join(folder_name, "ZAP_Ca_fit.txt")
    with open(file_path, 'w') as file:
...
#Ca plot
    point_size = 35
    plt.scatter(time, exp_data, s=point_size, c='none', edgecolors='red', marker='o', label='Expt')
    plt.plot(TT, MODEL, color='maroon', linewidth=4, label='Model')
...
#5. PSO
        ...
        data=pd.read_csv('SYKKO_ca1.dat',sep="\t", comment='#', header=None) #ZAP only
