...
# -*- coding: utf-8 -*-
"""
Created on Mon Mar  4 10:41:26 2024
...
def WT(n,K3,C1,C2,g,K1,KSA,KST,C1s,C2s,gs,time,exp_data):
...
#0. Bionetgen parameter set 
    ...
    with open("Model_WT.bngl", "w") as f:
...
# #1. Bionetge run : average PZAP   over n run and obseravble is in the (p+1) column
    ...
    T,avg_SYK=run_bionetgen_WT(n,p) #SYK
    T=np.asarray(T)
...
#2.Interpolate pSYKP  from 600 points to 2000 points   
    ...
    T,avg_ZAP=run_bionetgen_WT(n,p1) #SYK
    T=np.asarray(T)
...
#2.Interpolate PZAP  from 600 points to 2000 points   
    ...
    file_path = os.path.join(folder_name, "mixed_ZAP_SYK.txt")
...
#5. Ca Signal from the ODE model uisng the PSYK signal   ##########PSYK
   ...
   # Save data to a text file
    np.savetxt('ca_comp.txt', np.column_stack((tnew, ca_PZAP, ca_PSYK)), header='tnew ca_PZAP ca_PSYK', comments='', fmt='%1.5f')

    #Ticks
    manual_x_ticks = [0, 60, 120, 180, 240]
    manual_x_tick_labels = ['0', '60', '120', '180', '240']
    plt.xticks(manual_x_ticks, manual_x_tick_labels, fontsize=35)
    ...
    plt.plot(tnew,ca_PZAP, color='maroon', linewidth=6, label='Model')
    plt.plot(tnew,ca_PSYK, color='blue', linewidth=6, label='Model')
    plt.show()
  ...
  #7. Finding objective function only at t>60 sec
   ...
   # plt.plot(TT,EXPT,color=cmap(0.8),linewidth=3.0)
    ...
    # Write the data to a file in the folder
    file_path = os.path.join(folder_name, "mixed_Ca.txt")
    with open(file_path, 'w') as file:
        for i in range(len(time)):
            file.write(f"{time[i]} {exp_data[i]}\n")
            
     # Write the data to a file in the folder
    file_path = os.path.join(folder_name, "mixed_Ca_pred.txt")
    with open(file_path, 'w') as file:
...
#Ca plot
    ...
    plt.plot(TT, MODEL, color='black', linewidth=4, label='Model')
