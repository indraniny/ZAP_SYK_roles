...
def run_bionetgen(n,p,dir_name):
    ...
    for  i in range (0,n,1) :
          ret = bionetgen.run(dir_name+"_Model_ZAP70_SYK.bngl", out=dir_name+"_HPC_output_"+str(i),suppress=True)
     
    #save the time,pZAP70 data from folder_0
    dir_X= dir_name+"_HPC_output_0"
    X = open(dir_X+'/'+dir_name+'_Model_ZAP70_SYK.gdat').readlines()   # X is a list with  \t
    l=len(X)
    t=[]
    ...
    for i in range (1,n,1) : #reading folders i=1,n
              k=0
              dir_X=dir_name+"_HPC_output_"+str(i)
              X = open(dir_X+'/'+dir_name+'_Model_ZAP70_SYK.gdat').readlines()
