...
def SYK_only(n,K1,KSA,KST,C1s,C2s,gs,time,exp_data,cblb,dir_name):

    p=1 # which column index which obseravable in bionetgen file #total pSYK file
...
#0. Bionetgen parameter set 
    ...
    model.parameters.Z0 = 0 # setting parameter conc. of ZAP
    ...
    with open(dir_name+"_Model_SYK.bngl", "w") as f:
...
# #1. Bionetge run : average PZAP   over n run and obseravble is in the (p+1) column
    ...
    T,avg=run_bionetgen_SYK(n,p,dir_name) 
    T=np.asarray(T)
    avg=np.asarray(avg)
...
#5. Ca Signal   from the ODE model uisng the PZAP signal   
    y0 = [CA0, 1] #changed
    ca,h=solve(tnew,N,y0,PSYK,C1s,C2s,gs)
    ca=np.asarray(ca)
    h=np.asarray(h)
    # plt.plot(tnew,ca,'m')
    # plt.show()
  ...
  #7. Finding objective function only at t>25 sec
   ...
   #8. Delete existing bionetgen output folder and bionetgen model
    for i in range(0,n,1):
        path="/home/gddaslab/ixn004/ZAP_SYK_JAN_2024/FEB_RUNs_2024_kill_CD3z/cost_sum3d/"+dir_name+"_SYK_HPC_output_"+str(i)
        shutil.rmtree(path)

    os.remove("/home/gddaslab/ixn004/ZAP_SYK_JAN_2024/FEB_RUNs_2024_kill_CD3z/cost_sum3d/"+dir_name+"_Model_SYK.bngl")
    # for i in range(0,n,1):
    ...
    #print(exp_data[21])
    print("The (SSR)**0.25 is " + str(SSR_SYK**(1/4.0)))
    print('remove done')
